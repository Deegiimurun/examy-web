  ...
  const drawerItems = Object.keys(components).map(key => {
    return (
      <Link key={key} to={components[key].path} activeClassName={style.active}>
        <ListItem leftIcon={components[key].icon} caption={components[key].name} className={style.item} selectable />
      </Link>
    );
  });
    ...
    <aside className={classnames(style.root, style.navigation)}>
      <List className={style.list} selectable ripple>
        <IndexLink to="/" activeClassName={style.active}>
          <ListItem leftIcon="dashboard" caption="Тестын жагсаалт" className={style.item} selectable />
        </IndexLink>
        {drawerItems}
      </List>
      <footer className={style.footer}>
        <span className={style.footerTitle}>Д. Дэлгэрмөрөн</span>
        <span>Мэдээлэл комьпютерийн ухааны тэнхим</span>
      </footer>
    </aside>
  );
};
